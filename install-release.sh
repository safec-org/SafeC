...
# ──────────────────────────────────────────────────────────────────────────────
#  SafeC Prebuilt Install Script — macOS & Linux
#
...
#  you're installing to. This is the fast path; see install.sh for the
#  build-from-source path (needed for architectures/platforms the release
#  workflow doesn't publish, or to build from an unreleased commit).
#
#  Usage:  bash install-release.sh [OPTIONS]
#
#  Options:
...
Examples:
  ...
  bash install-release.sh --prefix=/opt/safec
  bash install-release.sh --version=v0.2.0
EOF
    exit 0
}
...
case "$os" in
    ...
    Linux)  plat_os="linux" ;;
    *)      die "Unsupported platform: $os (try install.sh to build from source)" ;;
esac
case "$arch" in
...
if [[ "$ASSET" != "safec-macos-arm64.tar.gz" && "$ASSET" != "safec-linux-x86_64.tar.gz" ]]; then
    die "No prebuilt release for ${plat_os}/${plat_arch} yet. Use install.sh to build from source instead."
fi
