...
void FuzzTarget::run(const unsigned char* corpus, unsigned long corpus_size) {
    ...
    unsafe {
        ...
        fn void(const unsigned char*, unsigned long) fuzz_fn =
            (fn void(const unsigned char*, unsigned long))self.func;
        fuzz_fn(buf, corpus_size);

        unsigned long i = (unsigned long)0;
        while (i < self.iters) {
            // Pick a random mutation: bit flip, byte set, truncate, or extend.
            unsigned long r   = fuzz_rng_((unsigned long*)&state);
            unsigned long mut = r & (unsigned long)3;
            unsigned long pos = fuzz_rng_((unsigned long*)&state) % corpus_size;

            if (mut == (unsigned long)0) {
                // Bit flip at random position.
                unsigned long bit = fuzz_rng_((unsigned long*)&state) & (unsigned long)7;
                buf[pos] = buf[pos] ^ (unsigned char)(1 << bit);
            } else if (mut == (unsigned long)1) {
                // Random byte set.
                buf[pos] = (unsigned char)(fuzz_rng_((unsigned long*)&state) & (unsigned long)0xFF);
                ...
                buf[pos2] = (unsigned char)(fuzz_rng_((unsigned long*)&state) & (unsigned long)0xFF);
            }

            // Vary the size: sometimes pass a prefix, sometimes full buffer.
            unsigned long sz = corpus_size;
            if ((fuzz_rng_((unsigned long*)&state) & (unsigned long)3) == (unsigned long)0) {
                sz = (fuzz_rng_((unsigned long*)&state) % corpus_size) + (unsigned long)1;
            }
