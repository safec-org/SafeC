...
inline unsigned long stack_len(&Stack s)      { return s.top; }
inline int           stack_is_empty(&Stack s) { return s.top == 0UL; }
inline void          stack_clear(&Stack s)    { s.top = 0UL; }
...
inline int stack_push(&Stack s, const void* elem) {
    unsafe {
        if (s.top == s.cap) {
...
inline int stack_pop(&Stack s, void* out) {
...
generic<T>
inline int stack_push_t(&Stack s, T val) {
    unsafe { return stack_push(s, (const void*)&val); }
}

generic<T>
inline T* stack_peek_t(&Stack s) {
    return (T*)stack_peek(s);
}

generic<T>
inline int stack_pop_t(&Stack s, T* out) {
